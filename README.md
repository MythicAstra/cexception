...
- Should not set the uncaught exception handler to a function that can return or to `NULL`.
...
- Directly jumping out of a TRY-CATCH block (use `INT_TRY`/`BREAK_TRY` to wrap the corresponding statement instead).
- A TRY-CATCH block does not terminate after executing `INT_TRY`/`BREAK_TRY`.
- Throwing an exception from a scope that contains any variable-length array, variably modified type object, or C++ automatic object with a non-trivial destructor.
...
| Identifier  | Parameters                 | Description                                                                                                               |
|-------------|----------------------------|---------------------------------------------------------------------------------------------------------------------------|
| `THROW_NEW` | `(type, message, deleter)` | Creates a new exception instance and throws it (`deleter`: `void (*)(void*)`, the function to delete `message`, nullable) |
| `THROW`     | `(exception)`              | Throws an exception                                                                                                       |
| `INT_TRY`   | `(statement)`              | Jumps out of the TRY-CATCH block by the given statement after trying to execute the `FINALLY` clause                      |
...
| Identifier          | Parameters                 | Description                                                             |
|---------------------|----------------------------|-------------------------------------------------------------------------|
| `NEW_EXCEPTION`     | `(type, message, deleter)` | Used to create a new exception instance (`deleter`: as mentioned above) |
| `DROP_EXCEPTION`    | `(exception)`              | Drops an exception instance (deletes `message`)                         |
...
| `deleter` | `void (*)(void*)`      | The deleter for `message`            |
...
|-----------------------------------|-------|--------------------------------------------------------------------------|
| `TryCatchStatusStart`             | `0`   | Starting a TRY-CATCH block                                               |
...
int main(void) {
    ...
    TRY {
        printf("TRY\n");
        number = 2;
        THROW_NEW(EXCEPTION, "an exception message", NULL);
        number = 3;
...
TRY
CATCH
Exception (file "example.c", line 11): an exception message
FINALLY
Number: 2
```
