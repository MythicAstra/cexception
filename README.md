...
- Shall not set the uncaught exception handler to a function that can return or to the null pointer.
- If an exception instance is not thrown, you need to drop it by using `DROP_EXCEPTION`.
...
Constants of the type `ExceptionType`.
...
Includes attributes of exception types.
...
The type of exception instances.
...
The type of TRY-CATCH contexts.
...
| `try_catch_context_stack_push`       | `void (TryCatchContext* context)`                                               | Adds a TRY-CATCH context to the TRY-CATCH context stack                         |
...
TRY
CATCH
EXCEPTION (file 'example.c', line 12): the exception message
FINALLY
Number: 2
```
