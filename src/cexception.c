...
static THREAD_LOCAL jmp_buf try_catch_interrupted_env;
...
void throw_exception(const volatile ExceptionInstance* exception) {
    ...
    if (try_catch_context_stack->status == TryCatchStatusTrying) {
        try_catch_context_stack->status = TryCatchStatusExceptionOccurred0;
    } else {
        try_catch_context_stack->status = TryCatchStatusExceptionOccurred1;
        DROP_EXCEPTION(try_catch_context_stack->exception);
    }
    try_catch_context_stack->exception = *exception;
    longjmp(try_catch_context_stack->env, 1);
}
...
jmp_buf* get_try_catch_interrupted_env(void) {
    return &try_catch_interrupted_env;
}
