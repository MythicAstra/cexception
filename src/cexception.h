...
#define TRY \
    { \
        ...
        bool _try_catch_finally_executed = false; \
        _try_catch_context.status = setjmp(_try_catch_context.env); \
        if (_try_catch_context.status == TryCatchStatusStart) { \
            _try_catch_context.status = TryCatchStatusTrying;

#define CATCH(type, variable) \
        } else if (_try_catch_context.status == TryCatchStatusExceptionOccurred && \
            exception_instance_of(&_try_catch_context.exception, &(type))) { \
            _try_catch_context.status = TryCatchStatusCaughtException; \
            const ExceptionInstance* const (variable) = &_try_catch_context.exception;
...
#define TRY_END \
        ...
        if (_try_catch_context.status == TryCatchStatusInterrupted) { \
            longjmp(_try_catch_context.env, 1); \
        } \
        if (_try_catch_context.status == TryCatchStatusExceptionOccurred || \
            _try_catch_context.status == TryCatchStatusExceptionRaised) { \
...
#define INTERRUPT_TRY(statement) \
    { \
        if (_try_catch_finally_executed) { \
            try_catch_context_stack_pop(); \
            statement; \
        ...
        { \
            jmp_buf _try_catch_context_env; \
            memcpy(_try_catch_context_env, _try_catch_context.env, sizeof(jmp_buf)); \
            if (setjmp(_try_catch_context.env) == 0) { \
                longjmp(_try_catch_context_env, TryCatchStatusInterrupted); \
            } \
        } \
        DROP_EXCEPTION(_try_catch_context.exception); \
        statement; \
...
#define BREAK_TRY(statement) \
    ...
    (void) 0
        ...
        #define NEW_EXCEPTION(type, message, deleter) \
            ((ExceptionInstance) {&(type), (message), (deleter), NULL, 0})
    #else
        #ifdef CEXCEPTION_DISABLE_LINE_INFO
            #define NEW_EXCEPTION(type, message, deleter) \
                ((ExceptionInstance) {&(type), (message), (deleter), __FILE__, 0})
        #else
            #define NEW_EXCEPTION(type, message, deleter) \
                ((ExceptionInstance) {&(type), (message), (deleter), __FILE__, __LINE__})
        #endif
    #endif
#else
    #ifdef CEXCEPTION_DISABLE_FILE_INFO
